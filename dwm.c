...
/* enum { SchemeNorm, SchemeSel, SchemeStatus, SchemeTagsSel, SchemeTagsNorm, SchemeInfoSel, SchemeInfoNorm }; /1* color schemes *1/ */
...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
		...
		/* do { */
		/* 	/1* do not reserve space for vacant tags *1/ */
...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		drw_text(drw, x, 0, w, bh, lrpad * 4 / 5, tags[i], urg & 1 << i);
		if (occ & 1 << i) 
        {
			//drw_circle(drw, x + boxs, boxs, 1.8*boxw, 
			//	m == selmon && selmon->sel && selmon->sel->tags & 1 << i,
			//	urg & 1 << i);
            int filled = (m == selmon && selmon->sel && selmon->sel->tags & 1 << i);
            if(filled)
            {
                drw_circle(drw, x + boxw, 3*boxs, boxw/2, boxw/2, lrpad/8, tags_indicator[0],urg & 1 << i);
            }
            else
                drw_circle(drw, x + boxw, 3*boxs, boxw/2, boxw/2, lrpad/8, tags_indicator[1],urg & 1 << i);
        }
